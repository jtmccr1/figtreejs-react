...
<svg width={600} height={400}>
    <FigTree  margins={{top:10,bottom:10,left:10,right:10}} width={600} height={400} tree={tree} >
        <Nodes filter={v=>!getNode(tree,v.id).children}  highlightOnHover={{r:10,fill:'#ae7e56',strokeWidth:2}} />
        <Branches />
    </FigTree>
</svg>
```
...
const tree = parseNewick(newickString,{labelName:"support"});	
const updatedTree = orderByNodeDensity(tree,true);
<svg width={600} height={400}>
    <FigTree  margins={{top:10,bottom:10,left:10,right:10}} width={600} height={400} tree={updatedTree} >
        <Nodes filter={v=>!getNode(tree,v.id).children}  highlightOnHover={{r:10,fill:'#ae7e56',strokeWidth:2}} />
        <Branches />
    </FigTree>
</svg>
